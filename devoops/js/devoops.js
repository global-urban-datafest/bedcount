...
function MorrisChart1(){
	var day_data = [
		{ "period": "Monday", "beds": 120, "trolleys": 32},
		{ "period": "Tuesday", "beds": 90, "trolleys": 20 },
		{ "period": "Wednesday", "beds": 70, "trolleys": 12 },
		{ "period": "Thursday", "beds": 80, "trolleys": 9 },
		{ "period": "Friday", "beds": 100, "trolleys": 60 },
		{ "period": "Saturday", "beds": 30, "trolleys": 4 },
		{ "period": "Sunday", "beds": 20, "trolleys": 10 }
	];
	Morris.Bar({
...
$(document).ready(function () {
	...
	setInterval(updatebedinfo, 1000);
});


function updatebedinfo() {
    var url = "http://ec2-79-125-15-15.eu-west-1.compute.amazonaws.com/latest";
    $.ajax({
        //mimeType: 'text/html; charset=utf-8', // ! Need set mimeType only when run from local file
        url: url,
        type: 'GET',
        success: function (data) {
            ...
            var runningTime = data.runningtime / 1000;
            var temperature = data.temp;
            var newHtml = '<button type="button" class="btn btn-success btn-app-sm btn-circle"><i class="fa fa-check"></i></button><br />B0001<br />Empty<br>' + temperature + ' deg C';
            if (data.occupancy > 0) {
                newHtml = '<button type="button" class="btn btn-info btn-app-sm btn-circle"><i class="fa fa-circle"></i></button><br />B0001<br />'
                    + runningTime + ' seconds <br>' + temperature + ' deg C';
            }
            if (runningTime > 10) {
                newHtml = '<button type="button" class="btn btn-warning btn-app-sm btn-circle"><i class="fa fa-bell-o"></i></button><br />B0001<br />'
                    + runningTime + ' seconds <br>' + temperature + ' deg C';
            }
            if (runningTime > 50) {
                newHtml = '<button type="button" class="btn btn-danger btn-app-sm btn-circle"><i class="fa fa-flash"></i></button><br />B0001<br />'
                    + runningTime + ' seconds <br>' + temperature + ' deg C';
            }
            $('.livebed').html(newHtml);
        },
        error: function (jqXHR, textStatus, errorThrown) {
            alert(errorThrown);
        },
